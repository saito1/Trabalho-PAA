...
def Prim(G = nx.Graph(), R = None):
    ...
    while Q:
        ...
        if predecessor[u] is not 'null':
            ...
            for v1,v2,peso in G.edges(peso=True):
                #Se v1 é predecessor de v2
                if (v1 is predecessor[u] and v2 is u):
                    MST.add_edge(v1,v2, weight=peso['weight']) #Adiciona a aresta formada por esses vértices assim como seu peso no novo grafo MST
                    H[i] = MST.copy() #Cria uma cópia do grafo MST para H que é um "vetor" de grafos para mostrar o passo a passo da função.
                    i = i + 1
                elif ((v1 is u and v2 is predecessor[u]) and (not nx.is_directed(G))): #Se acontecer ao cotrário.
                    MST.add_edge(v2,v1, weight=peso['weight'])
                    H[i] = MST.copy()
                    i = i + 1
...
def onclick(event):
    ...
    if toggle:
        labels = {}
        for v1,v2,data in G.edges(peso=True):
            labels[(v1,v2)] = peso['weight']
        nx.draw(G, posicao, with_labels=True)
        nx.draw_networkx_edge_labels(G, posicao, labels)
        toggle = not toggle
        
    #Senão, imprime o passo a passo do novo grafo encontrado.
    else:
        labels = {}
        for v1,v2,data in H[j].edges(peso=True):
            labels[(v1,v2)] = peso['weight']
        nx.draw(H[j], posicao, with_labels=True)
        nx.draw_networkx_edge_labels(H[j], posicao, labels)
        j = j + 1
